<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('personas.create') }}" class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">
                {{ __('Nueva Persona') }}
            </a>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="border border-gray-300 px-4 py-2">
                                            @forelse ($persona->intereses as $interes)
                                                <span class="inline-block bg-red-100 text-red-700 px-2 py-1 rounded text-sm mr-1">{{ $interes->nombre }}</span>
                                            @empty
                                                <span class="text-gray-500">{{ __('Sin intereses') }}</span>
                                            @endforelse
                                        </td>
                                        <td class="border border-gray-300 px-4 py-2">
                                            <a href="{{ route('personas.edit', $persona->id) }}" class="text-red-600 hover:text-red-800 hover:underline">{{ __('Editar') }}</a>
                    ...
                    @else
                        <p class="text-gray-600">{{ __('No hay personas registradas.') }} <a href="{{ route('personas.create') }}" class="text-red-600 hover:text-red-800 hover:underline">{{ __('Crear una') }}</a></p>
                    @endif
                </div>
            </div>
        </div>
